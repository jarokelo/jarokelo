...
var ReportsOnMap = (function() {
    ...
    var initMarkers = function() {
        $('.reportsonmap').find('.card').each(function() {
            ...
            if (site.Helper.isIE()) {
                ...
                mark.src = 'data:image/svg+xml;charset=UTF-8;base64,' + btoa(poi.replace('{{ color }}', color));
                mark.style.width = '32px';
                mark.style.height = '32px';
            }

            mark.title = $this.find('h3').text();
            mark.dataset.href = $this.find('h3').find('a').attr('href');
    ...
    function initMap() {
        ...
        if (window.reportsOnMapInitData.isHomeReferrer && MobileDetect.any()) {
            map.on('load', function () {
                $('.button-show_me-mobile').click();
            });
        }
    }
